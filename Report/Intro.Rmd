...
* `Data understanding` : In this study, we will use the `bands3.csv` database. We will analyse the available variables and correct possible errors. So we will look at the type, number and distribution of each variable and observation and deal with the many missing values so that they do not bias the analysis. 

* `Data preparation` : Using the exploratory data analysis (EDA), our goal will be to have an exploitable, consistent and homogeneous data base for the analysis of the models. We will then explore this data to find patterns that may explain the presence of grooves. We will also perform some operations that will allow us to have some clues about the origin of the problem. So in this section we will already have a small overview of the most influential variables.

* `Modeling` : For this part, we will try different models learned during the course. For each of them we will try different tuning parameters and calculate the accuracy. 

* `Evaluation` : The results obtained in the previous section will allow us to choose the most explanatory models depending on the accuracy and its ability to describe the pattern well. We will give the reasons for this as well as the implications for the future.

* `Conclusion and recommendations` : Finally, we will think about how to use our results in order to give the best advises to our clients concerning their banding problems and how they can reduce them.
